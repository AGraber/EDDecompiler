...
edzero_as_op_table = InstructionTable(GetOpCode, WriteOpCode, DefaultGetLabelName, CODE_PAGE)
...
InstructionNames[0x41] = 'AS_41'
InstructionNames[0x42] = 'AS_42'
...
class EDZEROASOpTableEntry(InstructionTableEntry):
...
def inst(op, operand = NO_OPERAND, flags = 0, handler = None):
    return EDZEROASOpTableEntry(op, InstructionNames[op], operand, flags, handler)
...
[
    ...
    inst(LoadEffect,                    'BBS'),              # LoadEffect(effId, effPath[:10])
    inst(FreeEffect,                    'W'),
    inst(AS_14,                         'W'),
    inst(WaitEffect,                    'BB'),
    inst(StopEffect,                    'BB'),
    inst(CancelEffect,                  'BB'),
    inst(PlayEffect,                    'BBBBhiiihhhhhhc'),      # PlayEffect(eff_owner, pos, eff_index, flag, x, z, y, degree_vert, degree_horz, unknown, size_x, size_z, size_y, eff_handle)
    inst(Play3DEffect,                  'BBSBBhiiihhhhhhc'),
    inst(AS_1A,                         'BBW'),
    inst(SetChrChip,                    'BB'),
    inst(DamageCue,                     'BB'),
    inst(DamageAnime,                   'BBL'),
    inst(AS_1E,                         'L'),
    inst(AS_1F,                         'WWB'),
    ...
    inst(AS_33,                         'BB'),
    inst(AS_34),
    inst(LockCamera,                    'Biiii'),       # LockCamera(target, x, z, y, move_time)
    inst(AS_36,                         'B'),
    inst(SetCameraDegree,               'hhhi'),        # SetCameraDegree(horizon, vertical, height, 0)
    inst(AS_3A,                         'Wi'),
    inst(SetCameraDistance,             'ii'),          # SetCameraDistance(Distance, MoveToTime)
    inst(AS_3C,                         'Wi'),
    inst(ShakeScreen,                   'iiii'),
    inst(AS_3E,                         'ii'),
    inst(AS_3F,                         'B'),
    inst(AS_40,                         'B'),
    inst(AS_41,                         'B'),
    inst(AS_42,                         'BBi'),
    inst(AS_43,                         'BLL'),
    inst(Fade,                          'BiL'),         # Fade(0 or 1, time, 0x0)
    inst(AS_45,                         'BL'),
    inst(AS_46,                         'BLL'),
    inst(AS_47,                         'B'),
    inst(AS_48,                         'BL'),
    ...
    inst(Knockback,                     'B'),
    inst(AS_59,                         'BW'),
    inst(SetBrightness,                 'BBi'),         # SetBrightness
    inst(ResetBrightness,               'i'),           # ResetBrightness(time)
    inst(ShowChr,                       'Bi'),
    inst(HideChr,                       'Bi'),
    inst(AS_5E,                         'B'),
    inst(AS_5F,                         'BB'),
    ...
    inst(AS_7D,                         'BL'),
    inst(AS_7E,                         'L'),
    inst(BlurSwitch,                    'LLLBL'),       # BlurSwitch(0, rgba, 0, 1, 3)
    inst(CancelBlur,                    'I'),
    inst(AS_82),
    inst(SortTarget,                    'B'),
    ...
    inst(LookingTargetAdd,              'BSW'),
    inst(LookingTarget,                 'ICCCC'),
    inst(AS_98,                         'B'),
    inst(AS_99,                         'B'),
    inst(AS_9A,                         'L'),
    inst(AS_9B,                         'B'),
...
for op in edzero_as_op_list:
    edzero_as_op_table[op.OpCode] = op
    op.Container = edzero_as_op_table


def DynamicAddInstruction(op, name, operand = NO_OPERAND, flags = 0, handler = None):
    entry = EDZEROASOpTableEntry(op, name, operand, flags, handler)
    edzero_as_op_list.append(entry)

    edzero_as_op_table[op] = entry
    entry.Container = edzero_as_op_table
...
if __name__ == '__main__':
    valid = 0
    for inst in edzero_as_op_list:
        if inst.OpName[:3] != 'AS_':
            valid += 1
    print('known: %d (%d%%)' % (valid, valid / len(edzero_as_op_list) * 100))
    print('total: %d' % len(edzero_as_op_list))
    console.pause()
